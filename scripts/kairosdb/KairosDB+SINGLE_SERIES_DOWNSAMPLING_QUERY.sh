# bash KairosDB+SINGLE_SERIES_DOWNSAMPLING_QUERY.sh > KairosDB+SINGLE_SERIES_DOWNSAMPLING_QUERY.txt
...
do
	...
	do
		...
		sleep 60s #这个不能太小，否则服务器还没启动完毕后面就查询了，调大是比较安全的
                ...
                java -jar /data/dbms/BenchTSDB/BenchTSDB-1.0-jar-with-dependencies.jar -r "/data/dbms/BenchTSDB/configs/kairosdb/KairosDB+SINGLE_SERIES_DOWNSAMPLING_QUERY_$i.properties" #note this should NOT be made background !!!!!!
        done
done
